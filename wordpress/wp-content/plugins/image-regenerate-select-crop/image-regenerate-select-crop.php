...
define( 'SIRSC_ASSETS_VER', '201711106.1450' );
...
class SIRSC_Image_Regenerate_Select_Crop {
	...
	private function init() {
		...
		if ( is_admin() ) {
			...
			self::$crop_positions = array( 'lt' => __( 'Left/Top', 'sirsc' ), 'ct' => __( 'Center/Top', 'sirsc' ), 'rt' => __( 'Right/Top', 'sirsc' ), 'lc' => __( 'Left/Center', 'sirsc' ), 'cc' => __( 'Center/Center', 'sirsc' ), 'rc' => __( 'Right/Center', 'sirsc' ), 'lb' => __( 'Left/Bottom', 'sirsc' ), 'cb' => __( 'Center/Bottom', 'sirsc' ), 'rb' => __( 'Right/Bottom', 'sirsc' ), );
	...
	public static function image_regenerate_select_crop_settings() {
		...
		$allow_html = array( 'table' => array( 'class' => array(), 'cellspacing' => array(), 'cellpadding' => array(), 'title' => array(), ), 'tbody' => array(), 'tr' => array(), 'td' => array( 'title' => array(), ), 'label' => array(), 'input' => array( 'type' => array(), 'name' => array(), 'id' => array(), 'value' => array(), 'checked' => array(), 'onchange' => array(), ), );
					...
					if ( ! empty( $filename ) ) :
						...
						<?php if ( ! empty( $all_size ) ) : ?>
							<table class="wp-list-table widefat fixed media">
								<thead>
									<tr>
										<th colspan="3"><?php esc_html_e( 'The original image', 'sirsc' ); ?>: <b><?php echo (int) $original_w; ?></b>x<b><?php echo (int) $original_h; ?></b>px, <?php esc_html_e( 'file size', 'sirsc' ); ?>: <b><?php echo self::human_filesize( $original_filesize ); ?></b>.</th>
									</tr>
									<tr>
							...
							foreach ( $all_size as $k => $v ) {
								...
								echo '<tr class="' . $cl . '" id="sirsc_recordsArray_' . (int) $post_data['post_id'] . $k . '">
									...
									<td class="image-size-column"><b>' . $k . '</b><br />(' . $size_text . ')<hr>' . esc_html__( 'Quality', 'sirsc' ) . ' <b>' . self::$settings['default_quality'][ $k ] . '%</b><br>' . esc_html__( 'File size', 'sirsc' ) . ' <b>' . self::human_filesize( $filesize ) . '</b></td>
						...
						foreach ( $rows as $v ) {
							...
							if ( ! empty( $filename ) ) {
								...
								if ( file_exists( $file ) && $file != $filename ) {
									...
									echo __( 'The image ', 'sirsc' ) . ' <b>' . $th_src . '</b> ' . esc_html__( 'has been deleted.', 'sirsc' );
									@unlink( $file );
									$unset = true;
									$deleted = true;
								} else {
									echo esc_html__( 'The image', 'sirsc' ) . ' <b>' . $th_src . '</b> ' . esc_html__( 'could not be deleted (it is the original file).', 'sirsc' );
								}
							}
							if ( $unset ) {
								...
								wp_update_attachment_metadata( $v['ID'], $image_meta );
								if ( ! $deleted ) {
									echo esc_html__( 'The image ', 'sirsc' ) . $_sisrsc_image_size_name . esc_html__( ' could not be found. ', 'sirsc' );
								}
							}
							-- $remaining_to_delete;
							$next_post_id = $v['ID'];
					...
					if ( ! empty( $rows ) && is_array( $rows ) ) {
						foreach ( $rows as $v ) {
							...
							if ( ! empty( $filename ) && file_exists( $filename ) ) {
								...
								echo esc_html__( 'Could not generate, the original file is missing.', 'sirsc' ) . '<hr />' . $filename;
							}
							echo '</center>';
							$next_post_id = $v['ID'];
						}
